...
public class Cowboy {
	...
	Cowboy(Model model, BufferedImage sprite, int rows, int columns, int x, int y, float scale) {
		m_model = model;
		m_sprite = sprite;
		m_ncols = columns;
		m_nrows = rows;
...
//			m_idx = (m_idx + 1) % m_sprites.length;
	...
	void paint(Graphics g) {
		if (m_explode) {
			...
			int w = (int) (m_scale * m_w);
			int h = (int) (m_scale * m_h);
			g.drawImage(img, m_x, m_y - h, w, h, null);
		}
	}
